...
def task(args):
    width,temperature,repeat,index,label,model_name=args
    ...
    source_copy=OUT/'source'/f'{source_hash}.py'
    ...
    try:
        ...
        if model_name=='v9':
            checkpoint=protocol['registry']['models']['large']
            model=base.load_model('large',protocol)
        else:
            import torch
            from src.ppo_objects_v8c import ObjectPolicy
            legacy=base.read(ROOT/'configs/ppo_objects_v8_exploratory.json')
            assert file_sha256(ROOT/legacy['checkpoint'])==legacy['sha256']
            checkpoint={'checkpoint':legacy['checkpoint'],'sha256':legacy['sha256'],'experiment_alias':'v8-single-preference-specialist'}
            torch.set_num_threads(1)
            ckpt=torch.load(ROOT/legacy['checkpoint'],map_location='cpu',weights_only=False)
            model=ObjectPolicy(ckpt['dimension'],ckpt['layers'])
            model.load_state_dict(ckpt['model_state_dict']);model.eval();model.requires_grad_(False)
        ledger=CandidateLedger(params,refs,reference)
        ...
        save(target,{'config':config,'result':result,'points':points,
             ...
             'b_C':refs[0],'b_R':refs[1],'checkpoint':checkpoint})
        check_protected()
...
if __name__=='__main__':
    ...
    p.add_argument('--model',choices=['v9','v8'],default='v9')
    a=p.parse_args()
    if a.mode=='prepare':prepare()
        ...
        jobs=[(a.width,a.temperature,r,i,a.label,a.model) for r in a.repeats for i in a.indices]
