"""Rebuild the complete frozen-V9, 12000-candidate report from v11 records.
...
def load_protocol():
    ...
    from run_ppo_budget_v11 import load_protocol as verified_protocol
    protocol=verified_protocol()
    assert protocol['models_retrained_this_round'] is False
    assert protocol['milp_reuse']['solver_rerun'] is False
    if protocol['protocol']!='ppo-budget-v11':
        raise ValueError('This report only accepts the new ppo-budget-v11 protocol')
...
def orchestration_summary(protocol):
    """Every new task needs actual, not just queued, execution provenance."""
    checks=[]
    for task in protocol['tasks']:
        if task['kind']!='PPO':continue
        path=OUT/f"actual_execution/{task['id']}.json"
        item=read(path);done=OUT/f"completed/{task['id']}.json"
        launcher=read(OUT/f"execution-{item['execution_id']}.json")
        ...
        if (item['task']!=task or item['status']!='completed' or item['pid']<=0
            or not item['start_utc'] or not item['end_utc']
            or item['threads']!=1 or item['device']!='cpu'
            or item['completed_sha256']!=file_sha256(done)
            or task['id'] not in launcher['tasks']
            or read(done)['execution_id']!=item['execution_id']):
            raise ValueError('Incomplete actual execution provenance')
        checks.append({'task_id':task['id'],'path':path.relative_to(ROOT).as_posix(),'sha256':file_sha256(path)})
    return {'actual_tasks':checks,'metadata_warnings':[]}


def concurrency_description(execution):
    return (f"新PPO批次的workers配置为{execution['workers']}，每个任务单线程；"
            '工作进程配置不等于所有时刻都同时满负荷，运行时间保留当时资源争用条件。')


def orchestration_provenance_text(execution,root_link):
    return ('全部48个新前沿任务都有独立的实际启动/完成记录，含PID、UTC起止、入口执行ID和完成文件哈希，见'
            f'[实际执行记录]({root_link}/output/pareto-ppo-budget-v11/actual_execution/)。'
            '所有动作日志以gzip无损压缩JSONL保存，核验时完整解压并检查哈希与CRC；压缩不改变动作或预算。')
...
def timing_description(execution, training):
    return ('新PPO计算采用CPU冻结推理，每进程1线程。'+concurrency_description(execution)+
            ...
            'MILP复用v9的五组单线程求解及原始耗时，本轮没有重新运行MILP。表中时间均保留原始墙钟观测，'
            '包括一次完整前沿的搜索、严格候选核验及档案维护；新PPO计时包含gzip无损压缩动作日志及偏好进度写盘，'
            '历史NSGA-II B2累计时间包含B1中途快照写盘，不因本轮复制档案或离线回放而追加时间。'
            '模型加载、前沿公共初始方案首次评价及最后预算快照导出不计入对应搜索段；'
            '任务总耗时与准备耗时另存。'
            f"Train-S、Train-L在v9阶段的历史训练耗时分别为{training['small']['training_seconds']:.3f} s和"
...
def budget_levels(protocol):
    ...
    if any(t.get('budgets',[t.get('budget',PPO_BUDGET)])!=[PPO_BUDGET] for t in tasks if t['kind']=='PPO'):
        raise ValueError('PPO cap must remain 21 times 12000')
    return b1,b2
...
def preference_budget_row(final, archive_id, task, index):
    ...
    if cap!=12000 or not isinstance(actual,int) or not 0<=actual<=cap:
...
def budget_reference_text(protocol):
    ...
    reuse_text=('用户保留NSGA-II预算并允许复用，仅将PPO提高至每偏好12000、完整前沿252000候选上限；本轮直接复用v4原12次演化的15份NSGA-II预算档案，'
...
def milp_optimality_verdict(original, normalized):
    ...
    return {'schema':'v11-report-reused-milp-handoff-v1','source_flags':source_flags,
...
def milp_reference_note(nonoptimal_count):
    ...
    return ('本轮复用的五组MILP参照均满足声明数值容差内的最优终止与交接判据，表中没有限时可行参照。'
...
def large_coverage_interpretation(pair_rows):
    ...
    if (complete and all(row['P_covers_N']==0. for row in rows)
        ...
        return (f'本轮大规模完整偏好网格的近似前沿比较仍落后于NSGA-II。B1下，平均约{uncovered:.1f}%的PPO点未被对应NSGA-II档案覆盖，'
                ...
                '此前v8在0.5/0.5单点、每次12000候选下的标量目标胜出，不能直接套用于本轮V9多偏好模型的完整前沿比较；'
                '两次实验的模型训练口径、种子/设备设置和评价对象不同，当前结果尚不能将差距定量归因于其中某一项。')
...
def parameter_revision_evidence(protocol, parameters):
    ...
    return {'capacity_by_instance':capacities,'risk_by_instance':risks,
            ...
            'models_retrained_this_round':False,'historical_v9_models_retrained':True,
            'historical_v4_training_hyperparameters_changed':False,
            'current_v9_uses_optimized_architecture_and_training':True}


def verify_preflight(protocol):
    preflight=read(OUT/'preflight.json')
    assert preflight['status']=='PASS' and preflight['not_formal_result']
    assert preflight['new_budget']==12000
    assert preflight['checkpoint_sha256']==protocol['registry']['models']['large']['sha256']
    result=preflight['result']
    assert result['candidate_budget']==1920 and result['actual_candidate_attempts']==1920
    assert result['solution_id']==preflight['same_1920_prefix_solution_id']
    assert file_sha256(OUT/result['action_log_path'])==result['action_log_sha256']
    return preflight
...
def build(publish=False):
    ...
    if revision is None or not protocol.get('nsga_reuse'):
        raise ValueError('This frozen-V9 budget delivery requires unchanged v4 parameters and explicit historical NSGA reuse')
    ...
    for task in protocol['tasks']:
        ...
        for budget in task.get('budgets',[PPO_BUDGET]):
            ...
            if task['kind']=='NSGA':
                from run_ppo_budget_v11 import verify_reused_nsga
            ...
            run_rows.append({'archive_id':archive_id,'instance':task['instance'],'model':task.get('model',''),
                             ...
                             'run_origin':'v4_reused' if task['kind']=='NSGA' else 'v11_new_inference',
...
'''
    ...
    if revision is not None:
        case_intro='本轮逐字节复用v4参数修订后的五个基础案例及八个敏感性版本，不更换实例、不重新绑定参考尺度；使用原冻结V9大小模型，以每偏好12000候选重跑全部PPO；NSGA-II与MILP均复用历史档案，本轮没有训练或新MILP求解。'
        model_intro=(f"Train-S与Train-L分别对应v9阶段训练并冻结的{aliases['small']}、{aliases['large']}。两模型在v9阶段分别使用原v4对应规模的24个训练实例从头训练，采用v8优化后的全对象PPO结构，并按预先固定的21偏好episode分配覆盖整张偏好网格。每实例只分配1至3个偏好，不声称每实例均见齐21偏好。训练结束仅保存预定最终检查点，随后冻结推理；旧模型、训练库与结果保留。v8结构曾依据固定Test-4反馈调整，本轮不重训，沿用模型同样不抹去这种测试知情设计。")
        ...
        training_completion='两份多偏好模型此前已在v9阶段使用原v4训练库从头训练；本轮保持冻结用于全部测试；网络及训练口径采用优化后的v8，不冒充原v4训练规格'
...
{'新版' if revision is not None else '原'}小规模实例`{small_data['instance_id']}`绑定b_C={small_refs[0]:.15g}、b_R={small_refs[1]:.17g}。本节仅比较当前冻结Train-S与MILP，不构建完整精确帕累托前沿。MILP每组单次求解，显式时限3600 s、相对gap目标0；原始变量、状态、界和墙钟时间均另存。PPO各偏好沿用已记录的3个原评估种子，模型沿用登记的V9冻结模型，本轮不重新训练。
...
B1下𝒞(N,P)与𝒞(P,N)分别为{t5[0]}和{t5[1]}，B2下分别为{t5[2]}和{t5[3]}。这些数值只描述两算法在本案例、当前编码与明确预算下发现的近似解集，不证明任何一方覆盖真实完整前沿。本轮两算法候选预算不相同，且候选尝试次数本身不等于目标评价次数或运行时间，判断投入需结合附表A2的完整前沿时间。
...
注：案例`{base_identity}`；Train-L；21组偏好，每组最多12000候选、每批8个；每情景3次运行联合获得的近似前沿。两子图各展示−30%、−20%、基准、+20%、+30%五个情景，每一点对应已保存且严格可行的实际方案。各情景分别联合、去重和非支配筛选，不平均点坐标，也不跨情景筛除点。散点不表示点间插值可行；横轴为成本模型单位，纵轴为模型风险指标。9情景共计划{len(scenario_records)}次运行，成功{scenario_success}次。
{sensitivity_variants_text}
...
这些案例和重复只支持当前模型、生成规则与预算下的描述性结论；3次重复不是跨实例样本。NSGA-II为历史档案而非新计时对照，其原16任务并发条件与新PPO批次不同，时间只作原始投入记录，不据此作公平速度排名。本轮不更新模型参数；沿用的V9模型在原训练阶段只使用TRAIN数据并保存预定最终检查点，评估期间未根据结果更换实例、微调检查点或追加某一情景预算。但优化架构在此前已反复参照Test-4表现，因此本轮不能表述为无测试知情调参的泛化验证，也不据三个重复声称统计显著。正式的多实例统计验证、更多训练种子及真实企业数据验证需另行安排。

## 附录A 可复现参数与交付记录

### A.1 冻结模型和参考尺度

{markdown_table(['方法','训练实例','seed','历史训练时间 (s)','检查点'],[[f'PPO（Train-{label}）',training_instances[model],training[model]['training_seed'],f"{training[model]['training_seconds']:.3f}",aliases[model]] for model,label in [('small','S'),('large','L')]])}
...
公式采用可直接编辑的LaTeX数学块，不使用公式图片；数学排版需由支持LaTeX的Markdown预览器显示。本轮仅以新预算重跑全部PPO，PT不重训；NSGA-II与MILP保留原记录并复用；公式展示仍保持可编辑的文字形式。

本轮协议为{protocol['protocol']}，协议SHA256为`{protocol_hash}`。原始实例/参数版本、公共参考方案和固定b见[instances]({output_link}/instances/)；全部前沿、方案、覆盖点计数、未舍入坐标及候选工作量保存在[实验目录]({output_link}/)中。表4对应table4_selected_solutions.csv；表5和表6共用coverage_pairs.csv；图2对应sensitivity_points.csv，可编辑脚本为[plot_pareto_sensitivity.py]({root_link}/plot_pareto_sensitivity.py)，另交付[SVG矢量图]({output_link}/figures/sensitivity.svg)及350 dpi PNG。交付包含48次新PPO完整前沿、复用的12次历史NSGA-II演化（15个预算档案）、15次新小规模PPO求解及复用的5组3600 s上限MILP原求解档案；基准敏感性3次为已有Train-L/Test-4记录的复用。

{orchestration_provenance_text(execution,root_link)}

仅重建已完成结果的表图和暂存文稿使用`py -B {ENTRY_SCRIPT.name}`，最终确认后以`--publish`写入正式Markdown；该入口会核验协议并逐方案回放，不启动训练或新增搜索。报告模板、实际入口、绘图脚本与本文的SHA256另存report_verification.json。MILP的原始与规范化source_flags、报告层derived判定、全部判据及来源哈希见[最优性交接记录]({output_link}/milp/report_optimality_verdicts.json)。历史V9训练入口为`train_ppo_frontier_v9.py`、历史MILP入口为`run_frontier_v9_milp.py`；本轮二者均不调用，冻结推理入口为`run_ppo_budget_v11.py`，其余运行与核验命令以本轮[协议文档]({root_link}/docs/ppo_budget_v11_protocol.md)为准。报告自身回放不替代独立原MILP全约束核验；只有独立交付审核与实际文件哈希一致时才报告整体完成。源码与JSON等字节由.gitattributes保护；克隆后须核验记录的SHA，不通过自动重算manifest掩盖换行或来源变化。
