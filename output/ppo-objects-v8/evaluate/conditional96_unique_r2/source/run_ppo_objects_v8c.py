...
def initialize(args):
    ...
    config={'args':vars(args),'exploratory':True,'operator_vocabulary':OPS,
            ...
            'sources':{p:sha(ROOT/p) for p in ('run_ppo_objects_v8c.py','src/ppo_objects_v7.py',
                        'src/ppo_objects_v7b.py','src/ppo_objects_v8.py','src/ppo_objects_v8c.py','src/ppo_unique_sampling.py','src/solution_utils.py','hazardous_waste_model.py','sample_params.py')}}
...
def train(args):
    ...
    model.eval();torch.save({'format':'conditional_objects_v8c','model_state_dict':model.cpu().state_dict(),
