...
from src.ppo_objects_v7 import ObjectState,SearchEnvironment,OPS,distributions
from src.ppo_objects_v8c import sample_batch
from src.ppo_objects_v8f import ObjectPolicy
...
def initialize(args):
    ...
    config={'args':vars(args),'exploratory':True,'operator_vocabulary':OPS,
            ...
            'sources':{p:sha(ROOT/p) for p in ('run_ppo_objects_v8f.py','src/ppo_objects_v7.py',
                        'src/ppo_objects_v7b.py','src/ppo_objects_v8.py','src/ppo_objects_v8c.py','src/ppo_objects_v8f.py','src/ppo_unique_sampling.py','src/solution_utils.py','hazardous_waste_model.py','sample_params.py')}}
...
def forward(policy,encoded,edges,actions):
    return policy(encoded[0],encoded[1],edges,encoded[2],actions)
...
def train(args):
    ...
    for update in range(args.updates):
        ...
        for epoch in range(args.epochs):
            ...
            for offset in range(0,len(buffer),args.minibatch):
                ...
                logits,value=forward(model,[x[idx] for x in features],edges[env_indices[idx]],actions[idx])
    ...
    model.eval();torch.save({'format':'conditional_objects_v8f','model_state_dict':model.cpu().state_dict(),
...
def evaluate(args):
    ...
    if args.checkpoint:
        ...
        assert ckpt['format']=='conditional_objects_v8f'
    ...
    config['sampling']='three separately parameterized conditional 512 heads, preceding choices and raw edge inputs; no cheapest insertion'
    ...
    if model is None:raise ValueError('Conditional experiment requires a checkpoint')
    config['object_heads_used']=args.object_ablation=='none'
    ...
    while attempts<args.budget:
        encoded=env.encode()
        width=min(args.width,args.budget-attempts)
        with torch.inference_mode():
            context=encode_policy(model,tensors([encoded],device),edge)
            if args.unique:
                forbidden=[]
                def score_fn(chosen):
                    score=model.head_logits(context,torch.tensor([chosen],device=device))[0].cpu().numpy()/args.temperature
                    if chosen and args.object_ablation=='uniform':score=np.zeros(512)
                    return score
                sampled=[sample_unique(state,env.plan,score_fn,rng,forbidden) for _ in range(width)]
            else:
                context=tuple(x.expand(width,*x.shape[1:]) for x in context)
                sampled=sample_batch(model,context,[state]*width,[env.plan]*width,rng,
                                     args.temperature,args.object_ablation)
        starting=(env.plan,env.solution,env.metrics);best=starting;best_action=None;block_trace=[]
        for action,masks,lp in sampled:
...
def main():
    ...
    parser.add_argument('--variant',choices=['f'],default='f')
    ...
    if args.masks=='processing':
        if args.variant!='f':raise ValueError('Processing masks require the unchanged b encoder')
        from src.ppo_objects_v8f import ObjectState
