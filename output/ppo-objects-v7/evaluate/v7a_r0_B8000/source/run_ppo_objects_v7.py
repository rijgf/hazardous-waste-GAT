...
OUT=ROOT/'output/ppo-objects-v7'
...
def initialize(args):
    ...
    config={'args':vars(args),'exploratory':True,'operator_vocabulary':OPS,
            ...
            'sources':{p:sha(ROOT/p) for p in ('run_ppo_objects_v7.py','src/ppo_objects_v7.py',
...
def train(args):
    ...
    for path in paths:
        ...
        envs.append(SearchEnvironment(state,plan,(.5,.5),args.horizon))
    ...
    config['training']={'seed':args.seed,'instances':24,'rollout_steps':args.rollout,'updates':args.updates,
        'episode_horizon':args.horizon,'epochs':args.epochs,'minibatch':args.minibatch,'lr':args.lr,
    ...
    for update in range(args.updates):
        ...
        for step in range(args.rollout):
            ...
            for i,env in enumerate(envs):
                ...
                reward,status=env.step(action);done=env.age>=args.horizon
