...
class PPOImprover:
    ...
    def train(self, save_path: str | Path | None = None, training_instances: List[PPOTrainingInstance] | None = None, progress_callback=None) -> Dict[str, List[float]]:
        ...
        for iteration in range(train_iterations):
            ...
            advantages = self._scalarize_advantages(advantages_vector, preferences_b)
    ...
    @staticmethod
    def _scalarize_advantages(advantages_vector: torch.Tensor, preferences: torch.Tensor) -> torch.Tensor:
        """Preserve fixed cost/risk preferences before common batch whitening.

        Rewards already use each instance's fixed objective references. Whitening
        the two GAE components separately would introduce batch-dependent weights.
        Keep the two-output critic unchanged and normalize only the scalar actor
        advantage, with the same sample-standard-deviation convention as before.
        """
        advantages = (advantages_vector * preferences).sum(dim=-1)
        return (advantages - advantages.mean()) / advantages.std().clamp_min(1e-6)
