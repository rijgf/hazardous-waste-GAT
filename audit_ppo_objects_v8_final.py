...
from src.ppo_objects_v8f import ObjectState as FeatureState,ObjectPolicy as FeaturePolicy
from src.ppo_objects_v8v import ObjectPolicy as CriticPolicy
...
def audit_trial(path,provenance=True):
    ...
    if args.get('variant')=='f':state_class=FeatureState
    ...
    if result['checkpoint_used']:
        ...
        assert ckpt['format'] in ('full_objects_v7','conditional_objects_v8c','conditional_objects_v8f','conditional_objects_v8v')
        conditional=ckpt['format'].startswith('conditional_objects_')
        ...
        model_class=FeaturePolicy if ckpt['format']=='conditional_objects_v8f' else (ConditionalPolicy if conditional else ObjectPolicy)
        if ckpt['format']=='conditional_objects_v8v':model_class=CriticPolicy
        model=model_class(ckpt['dimension'],ckpt['layers']);model.load_state_dict(ckpt['model_state_dict']);model.eval()
    ...
    excluded_mass=0.;seen=set();memory_key=None
    ...
    for start in range(0,len(actions),args['width']):
        ...
        if not args.get('memoize') or current_key!=memory_key:excluded_mass=0.;seen=set()
        ...
        for offset,item in enumerate(block,start+1):
            ...
            if provenance:
                ...
                if args.get('unique') or args.get('memoize'):
    ...
    return p,plan,result['metrics'],{'trial':folder.name,'sampled_actions':len(actions),
        ...
        'same_state_sampling_memory':args.get('memoize',False),
...
def main():
    ...
    # Reuse only sealed, unchanged, full candidate/probability audits; never a
    # precheck or a report whose audit implementation has since changed.
    cache={}
    for name,source in [('audit.json','audit_ppo_objects_v8.py'),
                        ('audit_pre_memory.json','output/ppo-objects-v8/audit_sources/final_before_memory.py'),
                        ('audit_final.json','audit_ppo_objects_v8_final.py')]:
        file=OUT/name
        if not file.exists():continue
        report=read(file)
        if report.get('status')!='PASS' or not report.get('action_provenance'):continue
        if report['audit_source_sha256']!=sha(ROOT/source):continue
        for record in report['trials']:
            if record.get('all_candidate_statuses_and_selection_replayed'):
                cache[record['trial']]=(record,name,sha(file))
    pending=[];reused=[]
    for path in paths:
        hit=cache.get(path.parent.name)
        if hit:
            record,name,report_hash=hit;config=read(path.parent/'config.json')
            valid=(record['result_sha256']==sha(path) and record['source_config_sha256']==sha(path.parent/'config.json')
                   and record['actions_sha256']==sha(path.parent/'actions.jsonl'))
            valid=valid and all(sha(path.parent/'source'/f)==h and (not f.startswith('src/') or sha(ROOT/f)==h) for f,h in config['sources'].items())
            valid=valid and sha(ROOT/'output/pareto-parameter-revision-v4/instances/Test-4.json')==config['input_sha256']
            if config.get('checkpoint'):valid=valid and sha(ROOT/config['checkpoint']['path'])==config['checkpoint']['sha256']
            if valid:
                result=read(path);data=read(ROOT/'output/pareto-parameter-revision-v4/instances/Test-4.json')
                params=params_from_json_data(data['params']);samples.append((deserialize_plan(result['plan']),result['metrics']))
                records.append(record);reused.append({'trial':record['trial'],'audit':name,'sha256':report_hash})
                continue
        pending.append(path)
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures=[pool.submit(worker,(path,not args.skip_provenance)) for path in pending]
    ...
    report={'status':'PASS','preserved_files':len(preserved),'action_provenance':not args.skip_provenance,
            'trials':records,'full_milp':replay,'reused_verified_records':reused,'audit_source_sha256':sha(Path(__file__))}
    filename='audit_selected_final.json' if args.id else 'audit_final.json'
