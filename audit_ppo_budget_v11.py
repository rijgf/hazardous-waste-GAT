"""Independent v11 delivery checks and original full-MILP replay of every saved plan.
...
from src.reproducibility import deserialize_plan,file_sha256,plan_sha256,derive_seed
...
def check_log(record):
    ...
    with gzip.open(path,'rt',encoding='utf8') as stream:
        events=[json.loads(line) for line in stream]  # Read EOF to validate gzip CRC.
...
def check_small_metadata(result,data,protocol,index,repeat):
    assert result['instance_id']==data['instance_id']
    assert result['instance_sha256']==data['instance_sha256']
    assert (result['b_C'],result['b_R'])==(data['b_C'],data['b_R'])
    assert result['reference_plan_sha256']==data['reference_plan_sha256']
    assert result['repeat']==repeat and result['seed']==protocol['small_seeds'][index][repeat]
    assert result['preference']==protocol['small_preferences'][index]
    assert result['checkpoint']==protocol['registry']['models']['small']


def check_front_seed(point,task,index):
    assert point['seed']==derive_seed(task['seed'],'preference',index,bits=32)
...
def audit(require_report=False):
    ...
    for name,item in protocol['registry']['models'].items():
        entry=run.v9.model_entry(name);assert item==entry
    ...
    for task in protocol['tasks']:
        ...
        for budget in task.get('budgets',[252000]):
            ...
            if task['kind']=='PPO':
                ...
                for index,point in enumerate(scalar):
                    check_front_seed(point,task,index)
                    logs.append(check_log(point))
                    check_plan(name,point['plan'],point['metrics'],point['solution_id'],tuple(point['preference']))
                assert sum(x['actual_candidate_attempts'] for x in scalar)==front['counts']['candidate_attempts']<=252000
    ...
    for index,preference in enumerate(protocol['small_preferences']):
        for repeat,seed in enumerate(protocol['small_seeds'][index]):
            ...
            check_small_metadata(result,instances['small-fixed'],protocol,index,repeat)
    ...
    assert len(logs)==1023
    from report_ppo_budget_v11 import orchestration_summary
    execution=orchestration_summary(protocol)
    assert len(execution['actual_tasks'])==48
    full={}
    ...
    if require_report:
        ...
        assert handoff['report_source_sha256']==file_sha256(ROOT/'report_ppo_budget_v11.py')
    ...
    result={'status':'PASS','protocol_sha256':protocol_hash,'models':model_checks,'fronts':checks,
            'independent_coverage_pairs':coverage_checks,'actual_execution':execution,
            ...
            'audit_dependency_sha256':{name:file_sha256(ROOT/name) for name in
                ('audit_scalar_advantage_v5.py','normalize_pareto_milp.py','run_frontier_v9_milp.py',
                 'report_ppo_budget_v11.py','src/solution_utils.py','hazardous_waste_model.py')},
