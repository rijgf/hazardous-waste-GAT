...
## 红线参数 NSGA-II 正式预算重跑（2026-09-10，已完成）

- 最新用户要求采用范围探索红线参数，按文稿原预算重跑NSGA-II并更新Markdown与PPO对比图。当前表5/6及NSGA计时使用 `output/pareto-nsga-red-budget/`，优先于下方v11复用旧NSGA的历史记录；PPO仍复用V9模型、v11每偏好12000结果，不重训、不重跑PPO/MILP、不更新Word。本轮没有自动提交推送授权。
- 锁定种群20、交叉率0.1、个体单分量变异率1.0、非末周期初始化服务概率0.5；算法核心、编码、修复、非支配选择未改动。各规模原单实例、固定b和正式NSGA种子不变。四规模各3次至40320，Test-4同次接续至120960，共12次演化、15份档案，均已完成，不重复启动。4工作进程、每进程1线程。
- 表5 B1三配对均N→P=0、P→N=1；B2为N→P=0.0324±0.0374、P→N=0.8226±0.2375，不能再称两档全部覆盖。大规模B1/B2单次完整前沿时间均值分别721.09/2044.25秒，B2累计含B1。PPO预算不同且计时批次并发不同，不作等投入或公平加速结论。
- 主图按各方法3次中固定归一化参照点(1.1,1.1)下最大HV选一次，全部真实方案按成本排序连线，另附3次原始配对；表格仍为全部3次配对均值±样本SD。红线参数来自Test-4低预算反馈探索，不能声称无测试知情参数选择。
- 入口 `run_nsga_red_budget.py`、`report_nsga_red_budget.py`，协议 `docs/nsga_red_budget_protocol.md`。终验 `py -B audit_nsga_red_budget.py --require-report` PASS：3230份去重方案完整原MILP矩阵0违规、27配对与文稿一致、5903个历史文件/PT哈希未变、181项测试通过。审核SHA256 `f00b9309e362edb18b169e0118de8d960611876234115530fcbe206edc2cbecf`；正式稿SHA256 `2a9b0348fc72e9e769580ffac0df138e283d6087312fbca4910bef04e285d5d5`。旧v11文稿审核哈希仅为历史记录，不用旧报告入口覆盖当前稿。
