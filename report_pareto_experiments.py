"""Rebuild versioned tables, replay evidence and Markdown from complete raw runs."""
...
def budget_response_interpretation(pair_rows):
    ...
    expected={(repeat,budget) for repeat in range(3) for budget in (40320,120960)}
    if set(matched)!=expected or any(row['status']!='valid' for row in matched.values()):
        return 'B1与B2存在缺失或无效配对，未据不完整覆盖记录判断追加预算收益。'
    differences=[]
    for repeat in range(3):
        before,after=matched[repeat,40320],matched[repeat,120960]
...
def build():
    ...
    output_link='../'+OUT.relative_to(ROOT).as_posix()
    preflight=read(OUT/'preflight.json')
    execution=read(OUT/'execution.json')
    assert preflight['protocol_sha256']==execution['protocol_sha256']==protocol_hash
    assert preflight['passed'] and execution['device']=='cpu' and execution['threads_per_worker']==1
    ...
    if revision is not None:
        from plot_pareto_sensitivity_variants import plot as plot_variants
        plot_variants(OUT)
...
两种新增版本的可编辑绘图脚本为[plot_pareto_sensitivity_variants.py](../plot_pareto_sensitivity_variants.py)，同时提供[折线SVG]({output_link}/figures/sensitivity_line.svg)、[平滑SVG]({output_link}/figures/sensitivity_smooth.svg)与350 dpi PNG；[绘图追溯记录]({output_link}/figures/sensitivity_variants.json)保存源数据哈希、原始节点及仅供可视化的插值坐标。
'''
    ...
    small_limited_text='；'.join(f"偏好{x['preference']}下，PPO的最小J为{x['ppo']['metrics']['weighted_objective']:.6f}，"
        f"MILP可行J为{x['milp']['metrics']['weighted_objective']:.6f}（{milp_status_label(x['milp'])}），相对该incumbent的差异为{display_difference(x['difference'])}%"
    ...
    training_instances={model:len(record['instance_episode_counts']) for model,record in training.items()}
    assert set(training_instances.values())=={24}
    capacity_reference=preflight['instances']['Test-4']
    capacity_low_reference=preflight['instances']['capacity-70']
    ...
    scenario_records=[fronts[f'PPO-{source}-large-r{repeat}-B40320']
    ...
    case_intro='小规模质量验证保留原固定实例；其余测试每种所需规模固定使用编号000的一个实例，不按测试结果更换。'
    model_intro=(f"Train-S与Train-L分别是用户确认冻结的{aliases['small']}、{aliases['large']}检查点，每个模型已在对应规模的24个训练实例上训练；本轮没有训练、微调或检查点择优。")
    reference_intro=f"实例生成种子为{small_data['instance_seed']}，初始参考方案种子为{small_data['reference_seed']}。"
    registry_path='configs/frozen_ppo_models.json'
    training_completion='训练已完成，本轮仅加载冻结参数'
    revision_intro=''
    capacity_bound=(f"本案例还可以给出超越参考方案的容量界：四类废物整个规划期的全系统总产量依次为{', '.join(f'{value:.3f}' for value in type_totals.values())}，"
        f"而−30%版本任一具备处理技术的设施在单期的对应处理能力至少为{', '.join(f'{value:.4f}' for value in low_capacities.values())}，即各类全期总量的1.4倍。"
        '由于期初库存为0、库存与处理量非负且满足质量守恒，任一设施任一期的某类处理量不可能超过该类全系统全期产生量。因此，本次五个能力水平的处理能力上限均为冗余约束，它们的真实可行域及原始成本—风险目标并未改变。若近似前沿不同，应解释为容量token改变冻结策略的有限搜索表现，而不是系统处理能力放松的优化收益；本轮不据此得出实际处理瓶颈下的敏感性结论。')
    ...
    if revision is not None:
        case_intro='本轮按用户预先指定的规则修订处理能力与共载风险两个参数字段，小规模表4和四个000号测试案例均建立新版本；基础网络、其他参数及编号选择不按测试结果改变。'
        model_intro=(f"Train-S与Train-L分别对应本轮重新训练后冻结的{aliases['small']}、{aliases['large']}。两种训练规模各24个实例均按同一参数修订规则建立新版本，训练与测试采用独立固定种子抽取共载参数；训练实例数、网络、全部训练超参数和训练种子保持原规格。训练结束后只做冻结推理，不按测试覆盖率选择或微调检查点；旧训练库、旧检查点与v3结果完整保留。")
        reference_intro=(f"来源实例生成种子为{small_data['instance_seed']}，新版实例为`{small_data['instance_id']}`。"
            '新版参考方案采用逐期服务、联合车辆—设施分配及确定性路线排序构造，分配器种子为0；经严格可行验证后重新绑定b，构造准备时间单独记录。原参考种子只作为来源信息保留，不冒充新版构造过程的随机种子。')
        registry_path='configs/ppo_models_parameter_revision_v4.json'
        training_completion='本轮先在修订后的训练库重新训练两模型，随后冻结用于全部测试；未修改训练超参数'
...
计算采用CPU冻结推理，每进程1线程，正式前沿批次{execution['workers']}个并发任务；MILP另以单线程任务运行。时间为共享计算资源下的实际墙钟观测，包含一次完整前沿的全部搜索、严格候选核验及档案维护，不是独占机器速度基准。PPO计时包含各偏好的进度写盘；NSGA-II B2累计时间也包含B1中途快照写盘。模型加载、公共初始方案首次评价及最后预算快照导出不计入对应搜索段时间，任务总耗时与准备耗时另行保存在completed和frontier_runs记录中。Train-S、Train-L{'本轮重新' if revision is not None else '历史一次性'}训练耗时分别为{training['small']['training_seconds']:.3f} s和{training['large']['training_seconds']:.3f} s，不计入本轮推理时间。
...
注：表中PPO为PPO-Transformer的简称。PPO每行对应3次运行中J最小的同一个实际方案，成本、风险与J不分别择优；并列按运行编号。PPO时间为3次完整搜索耗时之和。MILP时间沿用单次原求解运行的实际墙钟时间，不含之后离线数值规范化或独立审计；C、R与J来自同一个经验证的实际变量解。相对差异记为Δ_J，单位为%，正值表示PPO目标值高于MILP参照值，计算式为：

$$
\Delta_J(\%)=100\,\frac{{J_{{\mathrm{{PPO}}}}-J_{{\mathrm{{MILP}}}}}}{{J_{{\mathrm{{MILP}}}}}}
$$

†仅表示相对尚未证明最优的可行incumbent的差异，不是最优性差距；终止原因见状态列及原日志，最优性均依据求解日志而非是否接近某个已知目标判断。
...
L20采用Test-4唯一固定案例`{base_identity}`，PPO使用Train-L。每次完整PPO前沿同时与相同重复编号的NSGA-II B1、B2档案比较；B2继续B1演化，不是额外一套独立重复。两方向分别统计，不能将覆盖率理解为成本或风险降低比例。

**表5 大规模实例下PPO-Transformer与NSGA-II的双向覆盖率**

{grouped_table('方法',['B1','B2'],[['PPO-L vs NSGA-II',*t5]])}

注：PPO-L表示PPO（Train-L），固定案例为L20。B1、B2分别为40320、120960次尝试。每格为同一固定案例3次完整前沿配对的均值±样本标准差。P表示PPO前沿，N表示对应预算的NSGA-II前沿；N→P表示𝒞(N,P)，P→N表示𝒞(P,N)，从PPO角度前者越小、后者越大越有利。两组比较复用同一PPO档案，B2未给PPO增加重启。B1配对：{paired_status([row for row in pair_rows if row['instance']=='Test-4' and row['model']=='large' and row['nsga_budget']==40320])}；B2配对：{paired_status([row for row in pair_rows if row['nsga_budget']==120960])}。原始覆盖点数与分母见coverage_pairs.csv。失败配对记NA，若有失败则均值为有效配对条件下的统计，少于2个有效配对时SD记NA。
...
注：两行×四组规模，每组两方向，共八个数值列；N→P表示𝒞(N,P)，P→N表示𝒞(P,N)。每格仅针对一个固定实例上的3次配对运行，均值±标准差反映算法随机性，不反映实例间变异。P为该行PPO模型档案，N为相同案例NSGA-II B1档案，不是将两个PPO模型互作覆盖基准。配对状态为{paired_status([row for row in pair_rows if row['nsga_budget']==40320])}；空前沿失败配对记NA，不代填0或1。
...
注：案例`{base_identity}`；Train-L；21组偏好，每组60步×32候选；每情景3次运行联合获得的近似前沿。两子图各展示−30%、−20%、基准、+20%、+30%五个情景，每一点对应已保存且严格可行的实际方案。各情景分别联合、去重和非支配筛选，不平均点坐标，也不跨情景筛除点。散点不表示点间插值可行；横轴为成本模型单位，纵轴为模型风险指标。9情景共计划{len(scenario_records)}次运行，成功{scenario_success}次。
{sensitivity_variants_text}

{'处理能力五个水平的联合前沿及方案标识完全重合，按实际结果保留，不为制造差异追加参数调整或搜索预算。' if capacity_identical else '处理能力扰动下的曲线按实际求解结果展示，不通过改动未声明容量或追加搜索人为拉开差异。'} 预检中，基准参考方案最大处理能力利用率为{capacity_reference['max_processing_utilization']*100:.2f}%，处理能力下调30%版本自身参考方案的最大处理利用率为{capacity_low_reference['max_processing_utilization']*100:.2f}%；基准参考方案车辆最大利用率为{capacity_reference['max_vehicle_utilization']*100:.2f}%。上述利用率对应各版本预先固定的参考方案，不冒充所有最优方案的约束活跃性证明。

即使某项处理能力约束未活跃，设施token中的处理能力与利用率输入仍可能随该参数改变，进而影响冻结策略的动作概率和有限预算搜索轨迹。因此，能力扰动下前沿的变化不能自动解释为可行域放松带来的系统收益，需要与约束利用率、实际方案和策略输入响应区分。
...
这些案例和重复只支持当前模型、生成规则与预算下的描述性结论；3次重复不是跨实例样本。本轮{'按用户预先指定规则修订实例参数并重新训练，不是按本轮测试结果调参；评估期间' if revision is not None else ''}未按结果更换实例、调整训练超参数、微调检查点或追加某一情景预算。正式的多实例统计验证、更多训练种子及真实企业数据验证需另行安排。
...
训练参数保持已冻结的20轮×24个episode×24步，每轮24次更新；学习率0.0003、γ=0.95、GAE λ=0.90、截断0.20、熵系数0.02、价值损失系数0.50。网络为Full Transformer，48维嵌入、1层、4头、前馈96维、Dropout=0、最多896 token、8算子、512对象，偏好token开启。{training_completion}；两模型对应规范路径与哈希以[{registry_path}](../{registry_path})为准。
...
公式采用可直接编辑的LaTeX数学块，不使用公式图片；数学排版需由支持LaTeX的Markdown预览器显示。本次仅改变公式展示形式，不改变计算定义或实验数值。

本轮协议为{protocol['protocol']}，协议SHA256为`{protocol_hash}`。原始实例/参数版本、公共参考方案和固定b见[instances]({output_link}/instances/)；全部前沿、方案、覆盖点计数、未舍入坐标及候选工作量保存在[实验目录]({output_link}/)中。表4对应table4_selected_solutions.csv；表5和表6共用coverage_pairs.csv；图2对应sensitivity_points.csv，可编辑脚本为[plot_pareto_sensitivity.py](../plot_pareto_sensitivity.py)，另交付[SVG矢量图]({output_link}/figures/sensitivity.svg)及350 dpi PNG。总计48次正式PPO前沿、12次NSGA-II运行（产生15个预算档案）、15次小规模PPO求解及5次3600 s上限MILP求解；基准敏感性3次为已有Train-L/Test-4记录的复用。

仅重建已完成结果的表图与论文使用`py -B {ENTRY_SCRIPT.name}`；该入口会核验协议并逐方案回放，不启动训练或新增搜索。报告模板、实际入口、绘图脚本与本文的SHA256另存report_verification.json。{'新版完整复现依次执行run_parameter_revision.py的prepare-data、train、prepare、preflight、run、milp、normalize、report及audit阶段；已有训练和求解记录按身份锁复用，不覆盖旧参数版结果。独立终验须执行`py -B audit_parameter_revision.py --require-report`，此选项同时核验论文、表格、图形与原始记录，runner的默认audit阶段不包含文稿核验。' if revision is not None else '原计算阶段仍使用run_pareto_experiments.py的prepare、preflight、run入口，已有符合协议记录直接复用。'}源码与JSON等字节由.gitattributes保护；克隆后须核验记录的SHA，不通过自动重算manifest掩盖换行或来源变化。
...
'''
    ...
    write(OUT/'report_verification.json',{'passed':True,'formal_front_records':len(fronts),
          ...
          **report_sources,'parameter_revision_evidence':revision,
          ...
          'plot_variants_script_sha256':file_sha256(ROOT/'plot_pareto_sensitivity_variants.py') if revision is not None else None,
